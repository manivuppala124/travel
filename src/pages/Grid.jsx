    ...
    <Router>
      <div className="bg-light">
        ...
        <div className="container">
          <div className="row mb-3"> {/* Search bar container */}
            <div className="col-md-12"> {/* Full-width search bar */}
              <input
              ...
              />
              <Button variant="outline-success" className="float-end">Search</Button> {/* Styled search button at the end */}
            </div>
          </div>

          <div className="row">
            {cities
              .filter((city) => city.name.toLowerCase().includes(searchTerm.toLowerCase())) // Filter cities based on search term
              .map((city) => (
                <div className="col-md-3 mt-3" key={city.id}>
                  <Card style={{ width: '20rem', marginLeft: '20px' }} className="md-10">
                    ...
                    <Card.Body>
                      ...
                      <Card.Text>
                        {city.description} {/* Truncate description */}
                      </Card.Text>
                      {/* Pass city object as a prop to Link */}
                      <Link onClick={() => history.push(`/whereto?city=${city.name.toLowerCase()}`)}>
        ...
        <Switch>
          <Route path="/whereto" element={<WhereTo/>} /> {/* Add route for WhereTo */}
        </Switch>
      </div>
    </Router>
  );
